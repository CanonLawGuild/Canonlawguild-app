...
//   useEffect(() => {
//     // Make sure to configure django-cors-headers if this fetch fails
//     fetch('http://127.0.0.1:8000/api/trainings/')
//       .then(res => res.json())
//       .then((data: TrainingItem[]) => {
//         // Filter down by matching backend choice types
...
//   if (loading) return <p className="text-muted mt-md">Loading live scheduled sessions...</p>;
//   if (trainings.length === 0) return null; // Hide area if no live sessions are posted yet

//   return (
//     <div className="container mt-md">
//       <h3 className="mb-sm">Upcoming Scheduled Sessions</h3>
//       <div className="grid grid--single-column">
//         {trainings.map(item => (
//           <div key={item.id} className="card" style={{ borderLeft: '4px solid var(--accent-color, #0056b3)', padding: '1rem', marginBottom: '1rem' }}>
//             <h4>{item.title}</h4>
//             <p className="text-muted mt-xs">{item.description}</p>
//             <p className="mt-sm" style={{ fontSize: '0.85rem' }}>
//               📅 <strong>Schedule:</strong> {new Date(item.start_date).toLocaleDateString()} to {new Date(item.end_date).toLocaleDateString()}
//             </p>
//           </div>
//         ))}
//       </div>
...
//         ]}
//       />
//       <LiveTrainingList categoryType="Annual AGM" />
//     </>
//   );
// }
