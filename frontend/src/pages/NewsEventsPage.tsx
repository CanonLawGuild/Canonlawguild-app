...
//         <h2>News & Events</h2>
//       </div>
//       <p className="text-muted mt-sm">Stay informed with the latest updates, gatherings, and community announcements from the Canon Law Sodality.</p>

//       <div className="grid mt-md grid--single-column">
//         <section className="card card--accent">
//           <h3 className="section-subheading">Upcoming Highlights</h3>
//           <ul>
//             <li>Public lectures and scholarly presentations</li>
//             <li>Annual gatherings and member briefings</li>
//             <li>Publication launches and community events</li>
//           </ul>
//         </section>
//       </div>
//     </div>
...
// // Type definitions matching your database event collection properties
// type NewsEvent = {
//   id: number;
//   title: string;
//   description: string;
...
//   useEffect(() => {
//     // Synchronize your client-side auth context headers with your storage records
...
//         // Connects via your custom Axios wrapper instance targeting your Django endpoints
...
//   // Real-time item filtering matrix
//   const filteredItems = items.filter(item => {
//     const titleMatch = item.title?.toLowerCase().includes(searchQuery.toLowerCase()) || false;
//     const contentMatch = item.content?.toLowerCase().includes(searchQuery.toLowerCase()) || false;
//     const locationMatch = item.location?.toLowerCase().includes(searchQuery.toLowerCase()) || false;
...
//         Stay informed with the latest updates, gatherings, and community announcements streamed direct from the Canon Law Sodality repository database.
//       </p>

//       {/* Database Error Catch Banner */}
//       {errorMessage && (
...
//           <input 
//             type="text" 
//             placeholder="Query data records by headline, keywords, location..." 
//             value={searchQuery}
...
//       {/* State Render Controller */}
//       {loading ? (
//         <div style={styles.loadingWrapper}>
//           <div style={styles.skeletonCard}>
//             <p style={{ padding: '20px', color: '#718096', margin: 0 }}>Querying index directories from backend relational database...</p>
//           </div>
...
//               No current news or event records reside inside this database partition collection index.
//             </div>
//           ) : (
...
//                       backgroundColor: item.category === 'Event' ? '#feebc8' : '#e2e8f0', 
//                       color: item.category === 'Event' ? '#c05621' : '#4a5568' 
//                     }}>
//                       {item.category || 'General Notification'}
//                     </span>
//                     <span className="meta" style={{ color: '#718096', fontSize: '0.85rem' }}>
//                       Posted: {item.published_at ? new Date(item.published_at).toLocaleDateString() : 'Draft Status'}
//                     </span>
//                   </div>

//                   <h3 className="title" style={styles.itemTitle}>{item.title}</h3>
                  
//                   {/* Event Specific Sub-Metadata rows */}
//                   {item.category === 'Event' && item.event_date && (
//                     <div style={styles.metaLine}>
//                       📅 Scheduled Date: <strong>{new Date(item.event_date).toLocaleString()}</strong>
//                     </div>
//                   )}
//                   {item.location && (
//                     <div style={styles.metaLine}>
//                       📍 Venue/Location: <strong>{item.location}</strong>
//                     </div>
//                   )}

//                   <p style={styles.contentText}>
//                     {item.content || 'No text summary body synopsis index column supplied inside this document record framework row cell.'}
//                   </p>
...
// // Complete Object-driven Design Theme System
    ...
    <div className="container mt-md" style={{ fontFamily: 'system-ui, sans-serif', paddingBottom: '60px' }}>
      ...
      {loading ? (
        ...
        <>
          {filteredItems.length === 0 && !errorMessage ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#718096', border: '1px dashed #cbd5e0', borderRadius: '8px' }}>
              No current news or event records reside inside this database partition filter.
            </div>
          ) : (
            <div className="grid mt-md" style={styles.stackGrid}>
              {filteredItems.map(item => (
...
const styles: { [key: string]: React.CSSProperties } = {
  filterBar: { display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: '16px', alignItems: 'center', marginBottom: '24px', backgroundColor: '#f7fafc', padding: '16px', borderRadius: '8px', border: '1px solid #e2e8f0' },
  searchInput: { width: '100%', padding: '10px 14px', borderRadius: '6px', border: '1px solid #cbd5e0', fontSize: '1rem', outline: 'none', boxSizing: 'border-box' },
  tabButton: { padding: '8px 16px', borderRadius: '6px', border: 'none', fontWeight: '500', cursor: 'pointer', transition: 'all 0.2s' },
  loadingWrapper: { display: 'flex', flexDirection: 'column', gap: '16px' },
  skeletonCard: { backgroundColor: '#f7fafc', border: '1px solid #e2e8f0', borderRadius: '8px', height: '80px', display: 'flex', alignItems: 'center' },
  stackGrid: { display: 'flex', flexDirection: 'column', gap: '20px' },
  eventCard: { backgroundColor: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '24px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', display: 'flex', flexDirection: 'column', gap: '12px' },
  cardHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  categoryBadge: { padding: '4px 10px', borderRadius: '4px', fontSize: '0.75rem', fontWeight: '700', textTransform: 'uppercase' },
  itemTitle: { fontSize: '1.4rem', color: '#1a202c', margin: 0, fontWeight: '600' },
  dateBlock: { display: 'flex', flexDirection: 'column', gap: '4px', padding: '12px 16px', backgroundColor: '#f7fafc', borderRadius: '6px', border: '1px solid #edf2f7', fontSize: '0.9rem', color: '#2d3748' },
  contentText: { fontSize: '1rem', color: '#4a5568', lineHeight: '1.6', margin: '4px 0 12px 0' },
  actionBlock: { display: 'flex', justifyContent: 'flex-start' },
  primaryLinkBtn: { backgroundColor: '#1a202c', color: '#ffffff', padding: '10px 20px', borderRadius: '6px', fontWeight: '500', fontSize: '0.95rem' }
};
